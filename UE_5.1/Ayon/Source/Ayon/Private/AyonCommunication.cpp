...
TSharedPtr<IWebSocket> FAyonCommunication::Socket = nullptr;
int32 FAyonCommunication::Id = 0;

void FAyonCommunication::CreateSocket()
{
...
void FAyonCommunication::ConnectToSocket()
{
	// Handle delegates for the socket.
	Socket->OnConnected().AddStatic(&FAyonCommunication::OnConnected);
	Socket->OnConnectionError().AddStatic(&FAyonCommunication::OnConnectionError);
	Socket->OnClosed().AddStatic(&FAyonCommunication::OnClosed);
	Socket->OnMessage().AddStatic(&FAyonCommunication::OnMessage);
	Socket->OnMessageSent().AddStatic(&FAyonCommunication::OnMessageSent);
...
void FAyonCommunication::CloseConnection()
{
	Socket->Close();
}

bool FAyonCommunication::IsConnected()
{
	return Socket->IsConnected();
}

void FAyonCommunication::CallMethod(const FString Method, const TArray<FString> Args)
{
...
void FAyonCommunication::OnConnected()
{
	// This code will run once connected.
	UE_LOG(LogTemp, Warning, TEXT("Connected to web socket server."));
}

void FAyonCommunication::OnConnectionError(const FString & Error)
{
...
void FAyonCommunication::OnClosed(int32 StatusCode, const FString& Reason, bool bWasClean)
{
...
void FAyonCommunication::OnMessage(const FString & Message)
{
...
void FAyonCommunication::OnMessageSent(const FString& MessageString)
{
	// This code is called after we sent a message to the server.
	UE_LOG(LogTemp, Verbose, TEXT("Message sent: %s"), *MessageString);
}

void FAyonCommunication::HandleResult(TSharedPtr<FJsonObject> Root)
{
...
void FAyonCommunication::HandleError(TSharedPtr<FJsonObject> Root)
{
...
void FAyonCommunication::RunMethod(TSharedPtr<FJsonObject> Root)
{
